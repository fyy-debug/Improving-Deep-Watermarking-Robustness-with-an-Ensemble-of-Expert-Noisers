...
def train():
    ...
    net_Gen = torch.nn.parallel.DistributedDataParallel(net_Gen, device_ids=[FLAGS.local_rank],output_device=FLAGS.local_rank,broadcast_buffers=False)
...
def main(argv):
    ...
    while os.path.exists(FLAGS.out_dir + FLAGS.name + str(run) + "/"):
        run += 1
    FLAGS.out_dir = FLAGS.out_dir + FLAGS.name + str(run) + "_" + str(FLAGS.local_rank) + "/"
    os.mkdir(FLAGS.out_dir)
