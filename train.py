...
flags.DEFINE_string('resume_identity', None, 'resume from identity checkpoint')
...
def train():
    ...
    data_transforms = {
        'train': transforms.Compose([
            ...
            #transforms.RandomCrop(size=(FLAGS.image_size, FLAGS.image_size)),
            transforms.ToTensor(),
    ...
    if FLAGS.resume:
        ...
        del checkpoint
    elif FLAGS.resume_identity:
        start = 1
        epoch = 0
        checkpoint = torch.load(FLAGS.resume_identity)
        net_Encdec.load_state_dict(checkpoint['enc-dec-model'])
        optim_EncDec.load_state_dict(checkpoint['enc-dec-optim'])
        net_Dis.load_state_dict(checkpoint['discrim-model'])
        optim_Dis.load_state_dict(checkpoint['discrim-optim'])
    ...
    if FLAGS.pretrain_iter > 0 and FLAGS.resume_pretrain is None and FLAGS.resume_identity is None and FLAGS.resume is None:
    ...
    if FLAGS.necst_iter > 0 and FLAGS.resume is None and FLAGS.resume_identity is None:
        net_Necst.pretrain()
    net_Necst.eval()
    ...
    for i in range(start, FLAGS.iter):
        ...
        for step in range(FLAGS.n_gen):
            ...
            adv_dec_messages = net_Encdec.decoder(adv_images)
        ...
        optim_Dis.zero_grad()
        pred_real = net_Dis(images)
        encoded_images, adv_images, redundant_decoded_messages, adv_redundant_decoded_messages = net_Encdec(images,messages,net_Gen,net_Necst)
        pred_fake = net_Dis(encoded_images.detach())
        loss_fake, loss_real = dis_loss(pred_fake, pred_real)
        ...
        loss_ED = 0.01 * loss_fake + 1.5 * enc_dec_image_loss + 0.3 * enc_dec_message_encloss + 0.2 * enc_dec_message_advloss
        loss_ED.backward()
        optim_EncDec.step()
        ...
        if i == 1 or i % FLAGS.eval_every == 0 :
            ...
            with torch.no_grad():
                for images, _ in validation_loader:
                    ...
                    messages = torch.Tensor(np.random.choice([0, 1], (images.shape[0], FLAGS.message_length))).to(device)
                    for noise in noiser.noise_layers:
                        ...
                        redundant_decoded_messages = net_Encdec.decoder(noised_images)
                        decoded_messages = net_Necst.decode(redundant_decoded_messages)
                        decoded_rounded = decoded_messages.detach().cpu().numpy().round().clip(0, 1)
        ...
        if i == 1 or i % FLAGS.save_every == 0:
            ## saving checkpoint ##
            checkpoint = {
                'enc-dec-model': net_Encdec.state_dict(),
                'enc-dec-optim': optim_EncDec.state_dict(),
